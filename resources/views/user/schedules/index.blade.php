            ...
            @foreach($schedules as $schedule)
                ...
                <div class="bg-white dark:bg-gray-800 shadow rounded-lg overflow-hidden p-4 flex flex-col lg:flex-row justify-between h-auto space-y-4 lg:space-y-0 lg:space-x-4">
                    ...
                    <div class="bg-gray-100 dark:bg-gray-700 shadow rounded-lg overflow-hidden p-4 flex flex-col justify-between w-full lg:w-1/3" style="height: 250px; aspect-ratio: 1 / 1;">
                        ...
                        <form action="{{ route('user.schedules.upload', $schedule->id) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                            @csrf
                            <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Upload Check-In Image</label>
                            <input type="file" name="check_in_image" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:shadow-outline">
                            <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-2">Upload</button>
                        </form>
                    </div>
                    ...
                    <div class="bg-gray-100 dark:bg-gray-700 shadow rounded-lg overflow-hidden p-4 flex flex-col justify-between w-full lg:w-1/3" style="height: 250px; aspect-ratio: 1 / 1;">
                        ...
                        <form action="{{ route('user.schedules.upload', $schedule->id) }}" method="POST" enctype="multipart/form-data" class="mt-4">
                            @csrf
                            <label class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2">Upload Check-Out Image</label>
                            <input type="file" name="check_out_image" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:shadow-outline">
                            <button type="submit" class="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded mt-2">Upload</button>
                        </form>
                    </div>
                </div>
